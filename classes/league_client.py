...
class LeagueClient():
	...
	def set_lcu_arguments(self) -> None:
		...
		if not self.lcu_available():
			sys.exit('No ' + self._lcu_name + ' found. Login to an account and try again.')
	...
	def set_sessions(self) -> None:
		...
		'''
		self._lcu_session = base64.b64encode(f'riot:{self._lcu_token}'.encode('ascii')).decode('ascii')
		self._riot_session = base64.b64encode(f'riot:{self._riot_token}'.encode('ascii')).decode('ascii')
